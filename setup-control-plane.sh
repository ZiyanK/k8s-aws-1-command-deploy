...
sudo apt update
echo "Install kubeadm, kubelete and kubectl"
sudo apt install -y kubelet kubeadm kubectl
sudo apt-mark hold kubelet kubeadm kubectl

# Check if kubectl and kubeadm install correctly
echo "Check kubectl version and kubeadm version"
...
# Wait for kubeadm to setup correctly
sleep 3

# Copy kubenetes config file to a new location and giving current user permissions
echo "Copying kubernetes config file and giving user permissions"
sudo mkdir -p $HOME/.kube
sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
sudo chown $(id -u):$(id -g) $HOME/.kube/config

# Fix cluster
echo "Fix containerd SystemdCgroup value"
...
# Deploy CNI Weave net for networking
echo "Apply weave daemonset to provide networking between containers"
